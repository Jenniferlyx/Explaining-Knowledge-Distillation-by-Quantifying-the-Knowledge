...
suggested lr:
vgg:                                                                    resnet:
pretrained:         logsapce(-3, -4), epochs=150                        logsapce(-2, -3), epochs=150
without_pretrained: logsapce(-2, -3), epochs=150                        logsapce(-1, -2), epochs=150
...
def train_network(PATH, args):
    ...
    if args.model == 'vgg11':
        if args.mode == '_pretrain' or args.mode == '':
            model = vgg11_pretrained(args.classes, seed=args.seed)
        elif args.mode == '_without_pretrain':
        ...
        if args.mode == '_pretrain' or args.mode == '':
            model = alexnet_pretrained(args.classes, seed=args.seed)
        elif args.mode == '_without_pretrain':
        ...
        if args.mode == '_pretrain' or args.mode == '':
            model = ResNet18_pretrained(args.classes, seed=args.seed)
        elif args.mode == '_without_pretrain':
        ...
        if args.mode == '_pretrain' or args.mode == '':
            model = ResNet50_pretrained(args.classes, seed=args.seed)
        elif args.mode == '_without_pretrain':
        ...
        if args.mode == '_pretrain' or args.mode == '':
            model = ResNet101_pretrained(args.classes, seed=args.seed)
        elif args.mode == '_without_pretrain':
        ...
        if args.mode == '_pretrain' or args.mode == '':
            model = ResNet152_pretrained(args.classes, seed=args.seed)
        elif args.mode == '_without_pretrain':
    ...
    best_acc1 = max(acc1, best_acc1)
    save_checkpoint(
        {'epoch': epoch+1, 'state_dict': model.state_dict(), 'best_acc1': best_acc1, 'optimizer': optimizer.state_dict()}, save_dir)
